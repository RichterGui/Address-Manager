...
export default class PostgresUserRepository extends IUserRepository {
  ...
  async create({
    ...
    try {
      ...
      const query = 'INSERT INTO users (id, name, lastname, email, password) VALUES($1, $2, $3, $4, $5) RETURNING *';
  ...
  async login({email, password}) {
    ...
    const token = sign({hash: user.password}, process.env.SECRET_KEY)
    return {id: user.id, name: user.name, lastname: user.lastname, token: token}
    } catch (error) {
      throw error
    }
  }
